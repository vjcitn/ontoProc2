...
-   `bbop_sqlite_db_gz()` : retrieve and cache all filenames of Semantic
    SQL ontologies available, checking for updated content relative to
    cache via ETag
...
-   `improve_nodes()` : inject linefeeds for node names for graph, with
...
-   `print()` :
    ...
    Show method for SemsqlConn. Concise one-line summary displayed when
    a `SemsqlConn` object is auto-printed at the R prompt.
